...
### 下次優先處理（使用者實機回饋）
- **長筆記難以滑動**：打開內容較多的筆記後，上下滑動手勢容易被文字輸入／游標操作接管，難以順暢瀏覽全文。
- **筆記編輯介面太靠底部**：底部操作區與手機系統手勢／安全區距離太近，需要增加 safe-area 間距並重新檢查鍵盤開啟時的版面。
- 本輪只記錄問題，尚未修改程式；下次應先以長內容、鍵盤開啟／關閉及 Android 手勢導覽三種狀態實機驗證。
