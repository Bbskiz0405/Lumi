...
4. [ ] 任務時間、提醒與週期：日期仍可只填一天，進階時才展開開始時間、結束時間、提前提醒與重複。
5. [ ] 自然輸入強化：解析「週五下午三點開會一小時」「每週二運動」，並在儲存前顯示可修改預覽。
6. [ ] 常用範本與一鍵改期：例如課程、運動、繳費；未完成任務可快速移到明天／週末。
7. [ ] 分類統計只做輕量回顧，不把行事曆變成企業分析 Dashboard。
