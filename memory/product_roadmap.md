...
行事曆端到端健檢見 `memory/calendar_ux_audit_2026-07-30.md`。v0.4.71 已完成跨日／過夜、快速日期與時長、來源混排、同步可見性及完成任務清除手機日曆項目；v0.4.77 已完成本機提醒與任務時間。後續依「Schedule／搜尋／快速改期 → 重複系列 → 多日曆與同步重試」推進。

v0.4.72 新增「工時」工作區：共用月曆會依行事曆／工時／財務切換標點，工時第一版支援單日上下班、標準工時與月結餘。v0.4.81 完成可自訂的每日標準工時與公司固定休息制度，休息不需逐日輸入。後續依朋友實際規則評估多段班、CSV、時薪與加班倍率，不直接套用法律或公司假設。
...
4. [ ] 任務時間、提醒與週期：v0.4.77 已完成具體時間與本機提前提醒；週期規則與系列編輯尚未完成。提醒維持任務屬性，不另建 Reminder 類型。
