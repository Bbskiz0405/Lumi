...
- 實機驗收未升版的導覽批次：底部「行事曆／財務」高光與返回規則，以及行事曆／工時／財務三頁向上捲動時共用月曆收合、子頁列吸頂、切頁保留各自捲動位置。
- 驗收通過後再決定升版／產 APK；其後續推差異化路線 **D → A → B**，或先做上架 blocker「換裝置資料遷移」。

## 目前階段

- 版號 **0.4.81 — 標準工時＋固定休息制度**：休息屬公司制度而非每日輸入，預設標準 8:00／休息 60 分；預計達標、下班提醒、有效工時皆依標準工時＋固定休息計算，月摘要只結算已下班紀錄。
- 未升版導覽批次已完成程式碼與靜態檢查：財務維持底部獨立主功能；「行事曆」只記憶行事曆／工時；三工作區共用月曆改為可隨各頁內容收合，子頁列吸頂，各頁保存自己的收合位置。待 Android 實機手勢與版面驗收。
