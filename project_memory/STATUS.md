...
- **實機驗收記帳／財務分析拆頁**（未 bump 版號，程式碼已進 master）：
  ...
  - **DB migration v7 首次啟動會跑**，先在有資料的機器確認不掉資料，並跑一次匯出／匯入。
- 驗收通過再決定 build／升版。

## 目前階段

- 版號 **0.4.81 — 標準工時＋固定休息制度**（未 bump）。
- 桌面 `Lumi-0.4.81-calendar-scroll.apk` 是**拆頁之前**的 build，不含本批改動。
- 未升版累積中的改動有兩批：① 可收合共用月曆＋底部歸屬（前次，已 build 未驗收）；② 記帳／財務分析拆頁＋完整財務中心（本次，未 build）。
